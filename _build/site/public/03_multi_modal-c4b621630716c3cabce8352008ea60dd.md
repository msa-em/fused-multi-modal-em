...
In this tutorial we aim to show you how within just a few steps you can fuse your EELS/EDX maps with HAADF or similar elastic imaging modalities to improve chemical resolution. This is Tutorial 1 where we look at an atomic resolution HAADF and EDX dataset of DyScO$_3$. The multi-modal data fusion workflow relies on Python, but even with minimal to no coding experience one should be able to successfully implement this technique on their data after following this short tutorial. Within the code there are <10 lines that need to be adjusted and we describe in detail both here and in the Mathematical Overview section best practices for these adjustments.  Within a few minutes (under a minute with experience), datasets such as the one in this tutorial can be transformed into resolution enhanced publishable results. (Figure 4.1)
+++
